...
// const CLASS_NAMES = ['oyster', 'shell', 'cluster'];
 ...
 * @returns {Array} Array of detection objects with class names and bounding boxes
 */
export const detectImage = async (
  ...
  const boxes = [];
  const detections = []; // Array to return for counting
  ...
  for (let idx = 0; idx < selected.dims[1]; idx++) {
    ...
    const [x, y, w, h] = [
      (box[0] - 0.5 * box[2]) * xRatio * window.innerWidth / 640., // upscale left
      (box[1] - 0.5 * box[3]) * yRatio * window.innerHeight / 640., // upscale top
      box[2] * xRatio * window.innerWidth / 640., // upscale width
      box[3] * yRatio * window.innerHeight / 640., // upscale height
    ]; // keep boxes in maxSize range
    
    // Get class name from label index
    const className = CLASS_NAMES[label] || `Class_${label}`;
    
    // Box object for rendering
    boxes.push({
      label: label,
      probability: score,
...
// Export class names for use in other components if needed
export { CLASS_NAMES };